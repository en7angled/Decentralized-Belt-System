...
*Audience: auditors and protocol reviewers. Threat analysis + resolution log for every validator and minting policy. For the architecture context these attacks apply to, read [`onchain-architecture.md`](onchain-architecture.md) first.*
...
This audit covers the onchain smart contract architecture of the Decentralized BJJ Belt System.

**Fully analyzed in this audit** (see §Script-by-Script Analysis for per-script detail):

- **MintingPolicy** ([`src/lib/onchain-lib/Onchain/Validators/MintingPolicy.hs`](../src/lib/onchain-lib/Onchain/Validators/MintingPolicy.hs))
- **ProfilesValidator** ([`src/lib/onchain-lib/Onchain/Validators/ProfilesValidator.hs`](../src/lib/onchain-lib/Onchain/Validators/ProfilesValidator.hs))
- **RanksValidator** ([`src/lib/onchain-lib/Onchain/Validators/RanksValidator.hs`](../src/lib/onchain-lib/Onchain/Validators/RanksValidator.hs))
- **MembershipsValidator** ([`src/lib/onchain-lib/Onchain/Validators/MembershipsValidator.hs`](../src/lib/onchain-lib/Onchain/Validators/MembershipsValidator.hs))
- **Protocol** ([`src/lib/onchain-lib/Onchain/Protocol.hs`](../src/lib/onchain-lib/Onchain/Protocol.hs)) — data types and pure functions
- **LinkedList** ([`src/lib/onchain-lib/Onchain/LinkedList.hs`](../src/lib/onchain-lib/Onchain/LinkedList.hs)) — generic sorted linked list
- **Utils** ([`src/lib/onchain-lib/Onchain/Utils.hs`](../src/lib/onchain-lib/Onchain/Utils.hs)) — shared helper functions

**Added after this audit — not covered in the per-script analysis below, but the same core attack vectors (§1–2) apply and mint-time enforcement is inherited via the MintingPolicy above:**

- **AchievementsValidator** ([`src/lib/onchain-lib/Onchain/Validators/AchievementsValidator.hs`](../src/lib/onchain-lib/Onchain/Validators/AchievementsValidator.hs)) — follows the same `AcceptAchievement` / `Cleanup` pattern as Ranks / Profiles; cleanup is permissionless only when no valid datum exists.
- **OracleValidator** ([`src/lib/onchain-lib/Onchain/Validators/OracleValidator.hs`](../src/lib/onchain-lib/Onchain/Validators/OracleValidator.hs)) — admin-gated UTxO guard; see [`onchain-architecture.md` § Oracle Hub](onchain-architecture.md#oracle-hub-for-parameters) for its security model.
- **OracleNFTPolicy** ([`src/lib/onchain-lib/Onchain/Validators/OracleNFTPolicy.hs`](../src/lib/onchain-lib/Onchain/Validators/OracleNFTPolicy.hs)) — one-shot parameterized minting policy; uniqueness is guaranteed by requiring the seed `TxOutRef` to be spent.

These three warrant a dedicated audit pass before any change to their semantics.
...
This check appears redundant because PV independently validates its own output (L122-123). However, **PV has multiple redeemers** (`UpdateProfile` and `AcceptPromotion`). When RV forces the profile to be in `txInfoInputs`, it forces PV to run — but PV could run with **any** redeemer. If an attacker constructs a transaction that spends the profile with `UpdateProfile` (instead of `AcceptPromotion`) while spending the promotion with `PromotionAcceptance`:
- PV (`UpdateProfile`): outputs profile with new image but **old rank pointer** — PV passes
...
|---|---|---|---|
| **R1** | RanksValidator L75-76 | Profile output check | Prevents promotion consumption via alternative PV redeemer (e.g., `UpdateProfile`) |
...
| `UpdateProfile` | `PromotionAcceptance` | **Blocked by R1** — RV's profile output check rejects image-updated datum |
...
| UpdateProfile + AcceptInterval | Possible (same practitioner User NFT) | Safe — both authorized by the same user |
...
> Covers the four scripts audited in detail. AchievementsValidator, OracleValidator, and OracleNFTPolicy were added post-audit and are listed in §Scope; they need a dedicated pass before they appear here.
...
|---|---|---|---|
| `UpdateProfile` | Owner User NFT | Updated datum @ same address, same value | None |
