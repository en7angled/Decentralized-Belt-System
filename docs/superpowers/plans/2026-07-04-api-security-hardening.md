...
```
  ...
  **Note on `COUNT(*)`:** the spec suggested `COUNT(*)` for `/search` totals. `searchProjected` derives group totals from materialized lists (`length items`). Step 1's `normalizeLimitOffset` default does **not** reach this path — the `/search` route has no limit/offset param and `searchProjected` passed `Nothing` to the underlying `getProfiles`/`getAchievements`, and `searchRanksWithNames`/`searchPromotionsWithNames` built their own uncapped `select`. This was closed in the final-review fix wave: a `searchQueryLimit` (500) constant now hard-caps all five sub-queries (`getProfiles`/`getAchievements` via their limit/offset argument; the rank/promotion selects via an esqueleto `limit`). Group totals are correspondingly capped at `searchQueryLimit`. A `COUNT(*)`-based exact total remains a possible future refinement, out of scope here.
...
- `/search` group totals use `length` over lists that are now hard-capped at `searchQueryLimit` (500) rows per entity — the Step-1 pagination default does **not** reach the `/search` path (no limit/offset param), so the cap is applied directly in `searchProjected` (via the `getProfiles`/`getAchievements` limit/offset argument) and in `searchRanksWithNames`/`searchPromotionsWithNames` (via an esqueleto `limit`). Group totals are therefore "up to the cap". A `COUNT(*)`-based exact total is a possible future refinement.
