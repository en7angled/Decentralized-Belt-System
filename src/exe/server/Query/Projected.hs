...
getPractitionerProfile :: (MonadIO m) => ProfileRefAC -> m PractitionerProfileInformation
getPractitionerProfile profileRefAC = liftIO $ runSqlite chainsyncDBPath $ do
...
getOrganizationProfile :: (MonadIO m) => ProfileRefAC -> m OrganizationProfileInformation
getOrganizationProfile profileRefAC = liftIO $ runSqlite chainsyncDBPath $ do
  mProf <- P.getBy (UniqueProfileProjection profileRefAC)
  case mProf of
...
getProfilesCount :: (MonadIO m) => Maybe ProfileType -> m Int
getProfilesCount maybeProfileType = liftIO $ runSqlite chainsyncDBPath $ do
  cnt <- selectOne $ do
...
getProfiles :: (MonadIO m) => Maybe (C.Limit, C.Offset) -> Maybe C.ProfileFilter -> Maybe (ProfilesOrderBy, SortOrder) -> m [Profile]
getProfiles maybeLimitOffset maybeProfileFilter maybeOrder = liftIO $ runSqlite chainsyncDBPath $ do
  rows <- select $ do
    pp <- from $ table @ProfileProjection
...
getPromotions :: (MonadIO m) => Maybe (C.Limit, C.Offset) -> Maybe C.PromotionFilter -> Maybe (PromotionsOrderBy, SortOrder) -> m [Promotion]
getPromotions maybeLimitOffset maybePromotionFilter maybeOrder = liftIO $ runSqlite chainsyncDBPath $ do
  rows <- select $ do
...
getPromotionsCount :: (MonadIO m) => Maybe C.PromotionFilter -> m Int
getPromotionsCount maybePromotionFilter = liftIO $ runSqlite chainsyncDBPath $ do
  cnt <- selectOne $ do
    pr <- from $ table @PromotionProjection
    case maybePromotionFilter of
      Nothing -> pure countRows
...
getRanks :: (MonadIO m) => Maybe (C.Limit, C.Offset) -> Maybe C.RankFilter -> Maybe (RanksOrderBy, SortOrder) -> m [Rank]
getRanks maybeLimitOffset maybeRankFilter maybeOrder = liftIO $ runSqlite chainsyncDBPath $ do
  rows <- select $ do
    rp <- from $ table @RankProjection
...
getRanksCount :: (MonadIO m) => Maybe C.RankFilter -> m Int
getRanksCount maybeRankFilter = liftIO $ runSqlite chainsyncDBPath $ do
  cnt <- selectOne $ do
    rp <- from $ table @RankProjection
    case maybeRankFilter of
      Nothing -> pure countRows
...
getBeltTotals :: (MonadIO m) => m [(BJJBelt, Int)]
getBeltTotals = liftIO $ runSqlite chainsyncDBPath $ do
  rows <- select $ do
    rp <- from $ table @RankProjection
