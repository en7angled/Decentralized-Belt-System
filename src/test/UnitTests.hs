...
import TxBuilding.Validators (achievementsValidatorGY, profilesValidatorGY, ranksValidatorGY)
...
unitTests =
  testGroup
    "BJJ Unit Tests"
    [oracleDeploymentTests, oracleAdminTests, promotionTests, promotionSecurityTests, membershipTests, achievementTests, cleanupTests]
    ...
    orgAwardsAchievementTest TestInfo {..} = do
      ...
      (_txId, orgAC) <-
        bjjInteraction
          ctx
          (w1 testWallets)
          (CreateProfileWithRankAction (ProfileData "BJJ Academy" "A BJJ Academy" "ipfs://QmOrg") Organization creationDate White)
          Nothing
      waitNSlots_ 1
